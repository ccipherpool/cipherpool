// src/pages/SuperAdmin.jsx — v3: approve/reject + review modal + realtime notifications
...
const safeCount = async (query) => {
  try { const r = await query; if (r.error) return 0; return r.count ?? 0; } catch { return 0; }
};
...
export default function SuperAdmin() {
  ...
  const [stats, setStats] = useState({
    ...
    newToday: 0, liveMatches: 0, todayRevenue: 0, monthlyRevenue: 0, conversionRate: 0,
  });
  ...
  const fetchStats = useCallback(async () => {
    try {
      ...
      let todayCoins = 0;
      try { const {data:t}=await supabase.from("wallet_transactions").select("amount").gte("created_at",today.toISOString()).gt("amount",0); todayCoins=(t||[]).reduce((s,x)=>s+(x.amount||0),0); } catch {}

      let monthCoins = 0;
      try { const {data:t}=await supabase.from("wallet_transactions").select("amount").gte("created_at",month.toISOString()).gt("amount",0); monthCoins=(t||[]).reduce((s,x)=>s+(x.amount||0),0); } catch {}

      let conversionRate = 0;
      try { const p=await safeCount(supabase.from("tournament_participants").select("user_id",{count:"exact",head:true})); conversionRate=totalUsers>0?Math.round((p/totalUsers)*100):0; } catch {}

      setStats({ totalUsers, onlineUsers, bannedUsers, pendingVerifications:pendingVerif, totalTournaments, activeTournaments, totalMatches, liveMatches, totalCoins, totalReports, openTickets, newToday, todayRevenue:todayCoins, monthlyRevenue:monthCoins, conversionRate });
  ...
  const fetchUsers = useCallback(async () => {
    try {
      ...
      const wMap={};(w||[]).forEach(x=>{wMap[x.user_id]=x.balance;});
      setUsers((p||[]).map(u=>({...u,coins:wMap[u.id]||0,display_name:u.username||u.full_name||u.name||u.email?.split("@")[0]||"Inconnu"})));
    } catch(err){console.error("fetchUsers:",err);}
  }, []);

  const fetchAdmins    = useCallback(async()=>{ try{const{data,error}=await supabase.from("profiles").select("*").in("role",["admin","super_admin","founder","fondateur"]).order("created_at",{ascending:false}); if(error)throw error; setAdmins(data||[]);}catch(err){console.error(err);}}, []);
  const fetchTournaments=useCallback(async()=>{ try{const{data,error}=await supabase.from("tournaments").select("*").order("created_at",{ascending:false}).limit(50); if(error)throw error; setTournaments(data||[]);}catch{setTournaments([]);}}, []);
  const fetchReports   = useCallback(async()=>{ try{const{data,error}=await supabase.from("reports").select("*").order("created_at",{ascending:false}); if(error)throw error; setReports(data||[]);}catch{setReports([]);}}, []);
  const fetchLogs      = useCallback(async()=>{ try{const{data,error}=await supabase.from("admin_logs").select("*").order("created_at",{ascending:false}).limit(100); if(error)throw error; setLogs(data||[]);}catch{setLogs([]);}}, []);
  const fetchSystemConfig=useCallback(async()=>{ try{const{data}=await supabase.from("system_config").select("*").single(); if(data){setMaintenanceMode(data.maintenance_mode||false);setRegistrationEnabled(data.registration_enabled!==false);setTournamentsEnabled(data.tournaments_enabled!==false);}}catch{}}, []);
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        if(error||!data||data.role!=="super_admin"){navigate("/dashboard");return;}
        setProfile(data);
        await Promise.all([fetchStats(),fetchUsers(),fetchTournaments(),fetchReports(),fetchLogs(),fetchAdmins(),fetchSystemConfig()]);
  ...
  useEffect(() => {
    if(!profile) return;
    intervalRef.current=setInterval(()=>{fetchStats();fetchUsers();fetchTournaments();fetchReports();fetchLogs();},30000);
    return()=>clearInterval(intervalRef.current);
  }, [profile]);

  // ── REALTIME: nouveaux users pending ─────────────────────────
  useEffect(() => {
    if(!profile) return;
    const ch=supabase.channel("superadmin-pending-users")
      .on("postgres_changes",{event:"INSERT",schema:"public",table:"profiles"},(payload)=>{
        if(payload.new?.verification_status==="pending"){
          showMsg("success",`🔔 Nouvel utilisateur en attente: ${payload.new.full_name||payload.new.email||"Inconnu"}`);
  ...
  useEffect(() => {
    if(!profile) return;
    const upd=()=>supabase.from("profiles").update({last_seen:new Date().toISOString()}).eq("id",profile.id).then(()=>{});
  ...
  const banUser=async(userId,duration)=>{
    const b=new Date();
    if(duration==="24h")b.setHours(b.getHours()+24);
    else if(duration==="7d")b.setDate(b.getDate()+7);
    else if(duration==="30d")b.setDate(b.getDate()+30);
    else b.setFullYear(b.getFullYear()+10);
    try{
      const{error}=await supabase.rpc("ban_user",{target_user:userId,banned_until:b.toISOString(),banned_by:profile.id});
      if(error)throw error;
      showMsg("success","Utilisateur banni ✅");fetchUsers();fetchStats();setShowBanModal(false);
    }catch(err){showMsg("error",err.message);}
  };

  const unbanUser=async(userId)=>{
    try{const{error}=await supabase.rpc("unban_user",{target_user:userId});if(error)throw error;showMsg("success","Utilisateur débanni ✅");fetchUsers();fetchStats();}
    catch(err){showMsg("error",err.message);}
  };

  const deleteUser=async(userId)=>{
    if(!window.confirm("Supprimer définitivement cet utilisateur ?"))return;
    try{const{error}=await supabase.rpc("delete_user_complete",{target_user:userId});if(error)throw error;showMsg("success","Supprimé ✅");fetchUsers();fetchStats();}
    catch(err){showMsg("error",err.message);}
  };
  ...
  const grantCoins=async()=>{
    ...
    try{
      ...
      if(error)throw error;if(!data?.success)throw new Error(data?.error||"Erreur");
      showMsg("success",`${amount>0?"+":""}${amount} coins → ${selectedUser.display_name} (solde: ${data.new_balance})`);
      setGrantAmount("");setGrantReason("");setShowWalletModal(false);fetchUsers();fetchStats();
    }catch(err){showMsg("error",err.message);}
  };

  const updateTournamentStatus=async(tId,status)=>{
    try{const{error}=await supabase.from("tournaments").update({status}).eq("id",tId);if(error)throw error;showMsg("success","Statut mis à jour ✅");setShowTournamentModal(false);fetchTournaments();fetchStats();}
    catch(err){showMsg("error",err.message);}
  };

  const deleteTournament=async(tId)=>{
    try{const{error}=await supabase.rpc("delete_tournament_complete",{tournament_id:tId});if(error)throw error;showMsg("success","Tournoi supprimé ✅");fetchTournaments();fetchStats();}
    catch(err){showMsg("error",err.message);}
  };

  const resolveReport=async(reportId,action)=>{
    try{const{error}=await supabase.from("reports").update({status:"resolved",resolved_by:profile.id,resolved_action:action,resolved_at:new Date().toISOString()}).eq("id",reportId);if(error)throw error;showMsg("success","Rapport résolu ✅");fetchReports();fetchStats();}
    catch(err){showMsg("error",err.message);}
  };

  const updateSystemConfig=async()=>{
    try{const{error}=await supabase.from("system_config").upsert({maintenance_mode:maintenanceMode,registration_enabled:registrationEnabled,tournaments_enabled:tournamentsEnabled,updated_by:profile.id,updated_at:new Date().toISOString()});if(error)throw error;showMsg("success","Configuration sauvegardée ✅");}
    catch(err){showMsg("error",err.message);}
  };

  const manualRefresh=async()=>{
    await Promise.all([fetchStats(),fetchUsers(),fetchTournaments(),fetchReports(),fetchLogs(),fetchAdmins()]);
    showMsg("success","Données mises à jour ✅");
  };

  const filteredUsers=users.filter(u=>{
    const s=search.toLowerCase();
    const m=u.display_name?.toLowerCase().includes(s)||u.email?.toLowerCase().includes(s)||u.free_fire_id?.includes(s);
    if(filter==="all")    return m;
    if(filter==="admins") return m&&u.role==="admin";
    if(filter==="founders")return m&&["founder","fondateur"].includes(u.role);
    if(filter==="banned") return m&&u.role==="banned";
    if(filter==="pending")return m&&u.verification_status==="pending";
    return m;
  });

  if(loading)return(<div className="min-h-screen bg-[#030014] flex items-center justify-center"><div className="text-center"><div className="w-16 h-16 border-4 border-purple-500/20 border-t-purple-500 rounded-full animate-spin mx-auto mb-4"></div><p className="text-white/60">Chargement...</p></div></div>);
  ...
  return(
    <div className="min-h-screen bg-[#030014] text-white relative overflow-hidden">
      <div className="fixed inset-0 pointer-events-none">
        ...
        <div className="absolute inset-0" style={{backgroundImage:"linear-gradient(rgba(124,58,237,0.05) 1px,transparent 1px),linear-gradient(90deg,rgba(124,58,237,0.05) 1px,transparent 1px)",backgroundSize:"50px 50px"}}></div>
      </div>
      ...
      `}</style>
        ...
        <motion.div initial={{opacity:0,y:-20}} animate={{opacity:1,y:0}} className="sa-header flex justify-between items-center mb-6">
          <div>
            <h1 className="text-4xl font-black">
              ...
              <span className="ml-2 text-xs px-2 py-1 bg-gradient-to-r from-purple-600 to-cyan-600 rounded-full">v3.0</span>
            </h1>
          ...
          <div className="sa-header-btns flex gap-3 items-center">
            ...
            <button
              ...
              className="relative px-4 py-2.5 bg-[#11152b] border border-purple-500/30 rounded-xl text-white/70 hover:text-white hover:border-yellow-500/50 transition-all"
              title="Utilisateurs en attente">
              🔔
              {stats.pendingVerifications>0&&(
                <span className="absolute -top-2 -right-2 min-w-[20px] h-5 px-1 rounded-full bg-red-500 text-white text-[10px] flex items-center justify-center font-bold">
                  {stats.pendingVerifications}
                </span>
              )}
            </button>
            <Link to="/admin" className="px-5 py-2.5 bg-gradient-to-r from-purple-600 to-cyan-600 rounded-xl font-bold text-sm hover:opacity-90 transition-all hover:scale-105">PANEL ADMIN</Link>
            <button onClick={manualRefresh} className="px-5 py-2.5 bg-[#11152b] border border-purple-500/30 rounded-xl text-white/60 hover:text-white hover:border-purple-500 transition-all text-sm">⟲ RAFRAÎCHIR</button>
          </div>
        </motion.div>
        ...
        <AnimatePresence>
          {message.text&&(
            <motion.div initial={{opacity:0,y:-10}} animate={{opacity:1,y:0}} exit={{opacity:0}}
              className={`mb-4 p-4 rounded-xl border ${message.type==="success"?"bg-green-500/10 border-green-500/30 text-green-400":"bg-red-500/10 border-red-500/30 text-red-400"}`}>
              {message.text}
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <div className="sa-stats-grid grid grid-cols-4 lg:grid-cols-8 gap-3 mb-6">
          {[
            ...
            <motion.div key={i}
              ...
              onClick={item.label==="EN ATTENTE"?()=>{setActiveTab("users");setFilter("pending");}:undefined}
              className={`group relative bg-[#0a0a1a] rounded-xl p-4 overflow-hidden ${item.label==="EN ATTENTE"?"cursor-pointer":""}`}
        ...
        <div className="sa-revenue-grid grid grid-cols-2 gap-4 mb-6">
          {[
            ...
            <motion.div key={i} initial={{opacity:0,x:i===0?-20:20}} animate={{opacity:1,x:0}} whileHover={{scale:1.02}}
              className="relative bg-gradient-to-r from-purple-600/20 to-cyan-600/20 border border-purple-500/30 rounded-xl p-6">
        ...
        <div className="sa-tabs flex gap-1 mb-6 border-b border-purple-500/20 pb-3 overflow-x-auto">
          {[
            ...
            <motion.button key={tab.id} whileHover={{scale:1.05}} whileTap={{scale:0.95}}
              ...
              className={`px-3 py-2 text-xs font-bold whitespace-nowrap transition-all relative rounded-lg ${activeTab===tab.id?"text-purple-400 bg-purple-500/10":"text-white/40 hover:text-white hover:bg-white/5"}`}>
              {tab.label}
            </motion.button>
          ))}
        </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {/* DASHBOARD */}
          {activeTab==="dashboard"&&(
            <motion.div key="dashboard" initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} exit={{opacity:0}} className="grid md:grid-cols-2 gap-6">
              <div className="bg-[#0a0a1a] border border-purple-500/20 rounded-2xl p-6">
                <h2 className="text-sm font-bold text-white mb-4 flex items-center gap-2"><span className="w-1 h-4 bg-purple-500 rounded-full"></span> ACTIONS RAPIDES</h2>
                <div className="grid grid-cols-2 gap-3">
                  {[
                    {icon:"⏳",label:`EN ATTENTE (${stats.pendingVerifications})`,action:()=>{setActiveTab("users");setFilter("pending");},color:"yellow"},
                    ...
                    <motion.button key={i} whileHover={{scale:1.05,y:-2}} whileTap={{scale:0.95}} onClick={a.action}
              ...
              <div className="bg-[#0a0a1a] border border-purple-500/20 rounded-2xl p-6">
                <h2 className="text-sm font-bold text-white mb-4 flex items-center gap-2"><span className="w-1 h-4 bg-purple-500 rounded-full"></span> ACTIVITÉ RÉCENTE</h2>
                <div className="space-y-2 max-h-52 overflow-y-auto pr-1">
                  {logs.length===0?<p className="text-white/40 text-center py-4 text-sm">Aucune activité</p>
                  :logs.slice(0,10).map(log=>(
                    <div key={log.id} className="bg-[#11152b] rounded-lg p-2.5">
                      <p className="text-xs text-white"><span className="text-purple-400">{log.user_id?(users.find(u=>u.id===log.user_id)?.display_name||log.user_id?.slice(0,8)):"Système"}</span> — {log.action}</p>
              ...
              <div className="md:col-span-2 bg-[#0a0a1a] border border-purple-500/20 rounded-2xl p-6">
                <h2 className="text-sm font-bold text-white mb-4 flex items-center gap-2"><span className="w-1 h-4 bg-purple-500 rounded-full"></span> ANALYTIQUES <span className="text-white/20 text-xs font-normal ml-2">↻ auto 30s</span></h2>
            ...
            <motion.div key="users" initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} exit={{opacity:0}}>
              <div className="bg-[#0a0a1a] border border-purple-500/20 rounded-2xl p-6">
                <div className="flex gap-3 mb-5">
                  <input type="text" placeholder="Rechercher nom, email ou FF ID..." value={search} onChange={e=>setSearch(e.target.value)}
                ...
                <div className="overflow-x-auto">
                        ...
                        <motion.tr key={user.id} whileHover={{backgroundColor:"rgba(124,58,237,0.08)"}}>
                          ...
                          <td className="px-3 py-2.5">
                            {user.verification_status==="pending"?(
                              <span className="text-yellow-400 text-xs font-bold">⏳ EN ATTENTE</span>
                            ):user.verification_status==="approved"?(
                              <span className="text-green-400 text-xs font-bold">✅ APPROUVÉ</span>
                            ):user.verification_status==="rejected"?(
                              <span className="text-red-400 text-xs font-bold">❌ REFUSÉ</span>
                            ):user.role==="banned"?(
                              <span className="text-red-500 text-xs font-bold">🚫 BANNI</span>
                            ):(
                              <span className="text-white/40 text-xs">—</span>
                            )}
                          </td>

                          <td className="px-3 py-2.5"><span className="text-purple-400 font-bold text-sm">{(user.coins||0).toLocaleString()}</span></td>
                  ...
                  {filteredUsers.length>50&&<p className="text-center text-white/40 text-xs mt-3">Affichage 50/{filteredUsers.length} — affinez la recherche</p>}
                </div>
              </div>
            </motion.div>
          )}
            ...
            <motion.div key="admins" initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} exit={{opacity:0}}>
              <div className="bg-[#0a0a1a] border border-purple-500/20 rounded-2xl p-6">
                ...
                <div className="space-y-3">
                  {admins.length===0?<p className="text-white/40 text-sm">Aucun admin trouvé</p>
                  :admins.map(admin=>(
                    <motion.div key={admin.id} whileHover={{scale:1.01,x:4}}
                      ...
                      <div>
                        ...
                        <div className="flex gap-2 mt-1.5">
                          <span className={`px-2 py-0.5 text-xs rounded-full border ${admin.role==="super_admin"?"bg-yellow-500/20 text-yellow-400 border-yellow-500/30":"bg-cyan-500/20 text-cyan-400 border-cyan-500/30"}`}>{admin.role?.toUpperCase()}</span>
                          <span className="text-white/30 text-xs">{new Date(admin.created_at).toLocaleDateString("fr-FR")}</span>
                        </div>
                      </div>
                      {admin.role!=="super_admin"&&(
                        <div className="flex gap-2">
                          <button onClick={()=>updateUserRole(admin.id,"user")} className="px-3 py-1 bg-orange-500/20 text-orange-400 rounded-lg text-xs hover:bg-orange-500/30 transition-all">Rétrograder</button>
                          <button onClick={()=>deleteUser(admin.id)} className="px-3 py-1 bg-red-500/20 text-red-400 rounded-lg text-xs hover:bg-red-500/30 transition-all">Supprimer</button>
                        </div>
                      )}
                    </motion.div>
                  ))}
            ...
            <motion.div key="tournaments" initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} exit={{opacity:0}}>
              <div className="bg-[#0a0a1a] border border-purple-500/20 rounded-2xl p-6">
                ...
                <div className="space-y-3">
                  ...
                  :tournaments.map((t,i)=>(
                    <motion.div key={t.id} initial={{opacity:0,x:-20}} animate={{opacity:1,x:0}} transition={{delay:i*0.04}} whileHover={{scale:1.01,x:4}}
                      ...
                      <div>
                        ...
                        <div className="flex gap-2 mt-1.5 flex-wrap">
                          <span className={`px-2 py-0.5 text-xs rounded-full border ${t.status==="open"?"bg-green-500/20 text-green-400 border-green-500/30":t.status==="ongoing"?"bg-blue-500/20 text-blue-400 border-blue-500/30":t.status==="completed"?"bg-purple-500/20 text-purple-400 border-purple-500/30":"bg-gray-500/20 text-gray-400 border-gray-500/30"}`}>{t.status}</span>
                          <span className="px-2 py-0.5 bg-white/5 text-white/50 rounded-full text-xs border border-white/10">{t.current_players}/{t.max_players} joueurs</span>
                          <span className="px-2 py-0.5 bg-yellow-500/10 text-yellow-400 rounded-full text-xs border border-yellow-500/20">🏆 {t.prize_coins}</span>
                        </div>
                      </div>
                      <div className="flex gap-2">
                        <button onClick={()=>{setSelectedTournament(t);setTournamentStatus(t.status);setShowTournamentModal(true);}} className="px-3 py-1 bg-purple-500/20 text-purple-400 rounded-lg text-xs hover:bg-purple-500/30 transition-all">Modifier</button>
                        <button onClick={()=>{setTournamentToDelete(t);setShowDeleteConfirm(true);}} className="px-3 py-1 bg-red-500/20 text-red-400 rounded-lg text-xs hover:bg-red-500/30 transition-all">Supprimer</button>
                      </div>
                    </motion.div>
                  ))}
                </div>
          ...
          {activeTab==="reports"&&(
            <motion.div key="reports" initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} exit={{opacity:0}}>
              <div className="bg-[#0a0a1a] border border-purple-500/20 rounded-2xl p-6">
                ...
                {reports.length===0?<p className="text-white/40 text-center py-8 text-sm">Aucun rapport en attente 🎉</p>
                :<div className="space-y-3">{reports.map(r=>(
                  <motion.div key={r.id} whileHover={{scale:1.01}}
                    ...
                    <div className="flex justify-between items-start mb-2">
                      <div><p className="text-sm text-white">{r.reason||"Sans raison"}</p><p className="text-xs text-white/40 mt-1">{new Date(r.created_at).toLocaleString("fr-FR")}</p></div>
                      <span className={`px-2 py-0.5 text-xs rounded-full border ${r.type==="cheat"?"bg-red-500/20 text-red-400 border-red-500/30":r.type==="insult"?"bg-orange-500/20 text-orange-400 border-orange-500/30":"bg-blue-500/20 text-blue-400 border-blue-500/30"}`}>{r.type||"other"}</span>
                    </div>
                    <div className="flex gap-2">
                      {["warning","mute","ban","ignore"].map(action=>(
                        <button key={action} onClick={()=>resolveReport(r.id,action)}
                          className={`px-3 py-1 rounded-lg text-xs transition-all ${action==="ban"?"bg-red-500/20 text-red-400 hover:bg-red-500/30":action==="warning"?"bg-yellow-500/20 text-yellow-400 hover:bg-yellow-500/30":action==="mute"?"bg-orange-500/20 text-orange-400 hover:bg-orange-500/30":"bg-white/10 text-white/60 hover:bg-white/20"}`}>
              ...
              <div className="bg-[#0a0a1a] border border-purple-500/20 rounded-2xl p-6">
                ...
                <div className="grid grid-cols-3 gap-4 mb-6">
                  {[
                    ...
                    <motion.div key={i} whileHover={{scale:1.05}} className="bg-[#11152b] rounded-xl p-4 border border-purple-500/20 hover:border-purple-500 transition-all">
                ...
                <div className="space-y-2 max-h-[500px] overflow-y-auto pr-2">
                  ...
                  :logs.map((log,i)=>(
                    <motion.div key={log.id} initial={{opacity:0,x:-20}} animate={{opacity:1,x:0}} transition={{delay:i*0.02}}
                      ...
                      <p className="text-xs text-white"><span className="text-purple-400">{log.user_id?(users.find(u=>u.id===log.user_id)?.display_name||log.user_id?.slice(0,8)):"Système"}</span> — {log.action}</p>
                      <p className="text-xs text-white/30">{new Date(log.created_at).toLocaleString("fr-FR")}</p>
                      {log.details&&<pre className="text-xs text-white/20 mt-1 bg-black/20 p-1.5 rounded overflow-x-auto">{JSON.stringify(log.details,null,2)}</pre>}
                    </motion.div>
                  ))}
                </div>
              </div>
          ...
          {activeTab==="system"&&(
            <motion.div key="system" initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} exit={{opacity:0}}>
              <div className="bg-[#0a0a1a] border border-purple-500/20 rounded-2xl p-6">
                ...
                <div className="space-y-3 mb-6">
                  {[
                    ...
                    <div key={i} className="flex items-center justify-between p-4 bg-[#11152b] rounded-xl border border-purple-500/20 hover:border-purple-500 transition-all">
                      <div><p className="font-medium text-white text-sm">{c.label}</p><p className="text-xs text-white/40">{c.desc}</p></div>
                ...
                <motion.button whileHover={{scale:1.02}} whileTap={{scale:0.98}} onClick={updateSystemConfig}
        ...
        <AnimatePresence>
          ...
          {/* ✅ REVIEW MODAL — Dossier utilisateur */}
          {showReviewModal&&reviewUser&&(
            <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
              className="fixed inset-0 z-[999] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4"
              onClick={()=>{setShowReviewModal(false);setReviewUser(null);}}>
              <motion.div initial={{scale:0.95,opacity:0,y:10}} animate={{scale:1,opacity:1,y:0}} exit={{scale:0.95,opacity:0}}
                onClick={e=>e.stopPropagation()}
                className="w-full max-w-5xl bg-[#0a0a1a] border border-purple-500/30 rounded-2xl overflow-hidden shadow-2xl max-h-[90vh] overflow-y-auto">
                
                {/* Header */}
                <div className="flex items-center justify-between px-6 py-4 border-b border-purple-500/20 sticky top-0 bg-[#0a0a1a] z-10">
                  <div>
                    <h2 className="text-2xl font-black bg-gradient-to-r from-purple-400 to-cyan-400 bg-clip-text text-transparent">DOSSIER UTILISATEUR</h2>
                    <p className="text-white/40 text-sm">Vérification d'identité — {reviewUser.full_name||reviewUser.email}</p>
                  </div>
                  <button onClick={()=>{setShowReviewModal(false);setReviewUser(null);}}
                    className="px-4 py-2 rounded-lg bg-white/5 text-white/60 hover:bg-white/10 hover:text-white transition">Fermer ✕</button>
                </div>

                <div className="grid md:grid-cols-2 gap-6 p-6">
                  {/* LEFT: infos + actions */}
                  <div className="space-y-4">
                    <div className="bg-[#11152b] rounded-xl p-4 border border-purple-500/20">
                      <h3 className="text-sm font-bold text-purple-400 mb-3">INFORMATIONS</h3>
                      <div className="grid grid-cols-2 gap-3 text-sm">
                        {[
                          {l:"Nom complet",  v:reviewUser.full_name},
                          {l:"Email",        v:reviewUser.email},
                          {l:"Âge",          v:reviewUser.age},
                          {l:"Ville",        v:reviewUser.city},
                          {l:"Pays",         v:reviewUser.country},
                          {l:"FF ID",        v:reviewUser.free_fire_id},
                          {l:"Inscrit le",   v:reviewUser.created_at?new Date(reviewUser.created_at).toLocaleString("fr-FR"):null},
                          {l:"Statut",       v:reviewUser.verification_status, highlight:true},
                        ].map((f,i)=>(
                          <div key={i}>
                            <p className="text-white/30">{f.l}</p>
                            <p className={`font-medium ${f.highlight?"text-yellow-400 uppercase font-bold":"text-white"}`}>{f.v||"—"}</p>
                          </div>
                        ))}
                      </div>
                    </div>
                    ...
                    <div className="bg-[#11152b] rounded-xl p-4 border border-purple-500/20">
                      <h3 className="text-sm font-bold text-cyan-400 mb-3">ACTIONS ADMIN</h3>
                      <div className="flex flex-wrap gap-3">
                        <button onClick={()=>approveUser(reviewUser.id)}
                          className="flex-1 px-4 py-3 bg-green-500/20 text-green-400 border border-green-500/30 rounded-xl hover:bg-green-500/30 transition font-bold text-sm">
                          ✅ Approuver
                        </button>
                        <button onClick={()=>rejectUser(reviewUser.id)}
                          className="flex-1 px-4 py-3 bg-red-500/20 text-red-400 border border-red-500/30 rounded-xl hover:bg-red-500/30 transition font-bold text-sm">
                          ❌ Refuser
                        </button>
                        <button onClick={()=>{setSelectedUser(reviewUser);setShowBanModal(true);setShowReviewModal(false);}}
                          className="flex-1 px-4 py-3 bg-orange-500/20 text-orange-400 border border-orange-500/30 rounded-xl hover:bg-orange-500/30 transition font-bold text-sm">
                          🚫 Bannir
                        </button>
                      </div>
                    </div>
                  ...
                  <div className="space-y-4">
                    <div className="bg-[#11152b] rounded-xl p-4 border border-purple-500/20">
                      <h3 className="text-sm font-bold text-purple-400 mb-3">CARTE NATIONALE (CIN)</h3>
                      {reviewUser.id_card_url?(
                        <img src={reviewUser.id_card_url} alt="CIN" className="w-full max-h-72 object-contain rounded-lg border border-purple-500/20 bg-black/30"/>
                      ):(
                        <div className="w-full h-40 flex items-center justify-center bg-black/20 rounded-lg border border-dashed border-white/10">
                          <p className="text-white/30 text-sm">Aucune image fournie</p>
                        </div>
                      )}
                    </div>
                    <div className="bg-[#11152b] rounded-xl p-4 border border-cyan-500/20">
                      <h3 className="text-sm font-bold text-cyan-400 mb-3">SELFIE AVEC CARTE</h3>
                      {reviewUser.selfie_url?(
                        <img src={reviewUser.selfie_url} alt="Selfie" className="w-full max-h-72 object-contain rounded-lg border border-cyan-500/20 bg-black/30"/>
                      ):(
                        <div className="w-full h-40 flex items-center justify-center bg-black/20 rounded-lg border border-dashed border-white/10">
                          <p className="text-white/30 text-sm">Aucune image fournie</p>
                        </div>
