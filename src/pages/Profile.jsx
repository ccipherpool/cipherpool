    ...
    <div style={{maxWidth:660}}>
      ...
      <div style={{marginTop:20}}>
        <button
          ...
          onMouseEnter={e=>{e.currentTarget.style.background="rgba(220,38,38,0.16)";e.currentTarget.style.borderColor="rgba(220,38,38,0.5)";}}
...
export default function Profile() {
  ...
  const viewingId = searchParams.get("id"); // if set = viewing another user's profile
  const isViewingOther = !!(viewingId && viewingId !== ap?.id);
    ...
    <>
      ...
      {isViewingOther && (
        <div style={{
          ...
          <span>Profil de <strong style={{color:"#fff"}}>{profile?.full_name || "..."}</strong></span>
          <span style={{flex:1}}/>
          <button onClick={()=>window.history.back()} style={{
            background:"rgba(124,58,237,0.15)",border:"1px solid rgba(124,58,237,0.3)",
            color:"#a78bfa",padding:"5px 14px",borderRadius:7,fontSize:12,
