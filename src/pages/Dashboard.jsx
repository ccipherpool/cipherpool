...
function StatCard({ icon, label, value, suffix="", accent, delay=0, onClick }) {
  return (
    <motion.div className="stat-card" style={{"--accent":accent}}
      ...
      <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:"clamp(24px,4vw,40px)",fontWeight:900,lineHeight:1,color:"#fff",marginBottom:4}}>
      ...
      <div style={{display:"flex",height:64}}>
        ...
        <div style={{flex:1,padding:"8px 12px",display:"flex",flexDirection:"column",justifyContent:"space-between",minWidth:0}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8}}>
            <p style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:13,fontWeight:800,lineHeight:1.1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{t.name}</p>
            {t.status==="in_progress"&&<span style={{display:"flex",alignItems:"center",gap:4,flexShrink:0,fontFamily:"'Share Tech Mono',monospace",fontSize:7,letterSpacing:2,color:"#7c3aed"}}><span style={{width:5,height:5,borderRadius:"50%",background:"#7c3aed",animation:"blink 1s infinite"}}/> LIVE</span>}
          </div>
          <div>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}>
              <span style={{fontFamily:"'Share Tech Mono',monospace",fontSize:8,color:"rgba(255,255,255,0.25)",letterSpacing:1}}>{t.current_players??0}/{t.max_players}</span>
    ...
    <div className="db-wrap">
      ...
      <div className="db-content">
        ...
        <motion.div initial={{opacity:0,y:-16}} animate={{opacity:1,y:0}}
          ...
          <div>
            <p style={{fontFamily:"'Share Tech Mono',monospace",fontSize:10,letterSpacing:3,color:"rgba(255,255,255,0.3)",marginBottom:6,textTransform:"uppercase"}}>
              BONJOUR · {timeStr}
            </p>
            <h1 style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:"clamp(32px,6vw,64px)",fontWeight:900,lineHeight:.95,textTransform:"uppercase",letterSpacing:-1}}>
          ...
          <div className="db-header-actions" style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}}>
            {profile?.verification_status==="approved"&&(
              <div style={{display:"flex",alignItems:"center",gap:6,fontFamily:"'Share Tech Mono',monospace",fontSize:10,letterSpacing:2,color:"#22c55e",padding:"6px 12px",background:"rgba(34,197,94,0.06)",border:"1px solid rgba(34,197,94,0.2)",borderRadius:6}}>
                ✓ VÉRIFIÉ
              </div>
            )}
            <div style={{display:"flex",alignItems:"center",gap:8,fontFamily:"'Barlow Condensed',sans-serif",fontSize:15,fontWeight:800,padding:"8px 14px",background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:6}}>
              <span style={{color:"#fbbf24"}}>💎</span>
              {coins.toLocaleString()}
              <span style={{fontFamily:"'Share Tech Mono',monospace",fontSize:9,color:"rgba(255,255,255,0.3)",letterSpacing:1}}>CP</span>
            </div>
            ...
            <div>
              ...
              {tournaments.length===0?(
                <motion.div initial={{opacity:0}} animate={{opacity:1}} style={{textAlign:"center",padding:"48px 16px",border:"1px solid rgba(255,255,255,0.04)",background:"rgba(255,255,255,0.01)",borderRadius:8}}>
                  ...
                  <p style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:16,fontWeight:800,color:"rgba(255,255,255,0.2)",letterSpacing:2,marginBottom:12}}>ARÈNE EN VEILLE</p>
                  <p style={{fontFamily:"'Share Tech Mono',monospace",fontSize:10,color:"rgba(255,255,255,0.2)",letterSpacing:1,marginBottom:20}}>Aucun tournoi actif</p>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:20}}>
            ...
            <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:.35}}
              ...
              <div style={{display:"flex",alignItems:"baseline",gap:10,marginBottom:16}}>
                <span className="db-rank-num" style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:60,fontWeight:900,lineHeight:1,color:stats.rank?"#fff":"rgba(255,255,255,0.08)"}}>
              ...
              {(()=>{
                ...
                return(
                  <div style={{display:"inline-flex",alignItems:"center",gap:8,background:`${r.color}10`,border:`1px solid ${r.color}30`,padding:"6px 12px",fontFamily:"'Barlow Condensed',sans-serif",fontSize:12,fontWeight:800,letterSpacing:2,color:r.color,borderRadius:4}}>
                    {r.label}
                  </div>
                );
              })()}
                ...
                {QUICK_LINKS.map((l,i)=>(
                  <motion.div key={l.label} className="ql"
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:10}}>
                      ...
                      <div>
                        <p style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:14,fontWeight:800,letterSpacing:.5,margin:0}}>{l.label}</p>
                        <p style={{fontFamily:"'Share Tech Mono',monospace",fontSize:9,color:"rgba(255,255,255,0.3)",letterSpacing:1,margin:0}}>{l.sub}</p>
                      </div>
                    </div>
