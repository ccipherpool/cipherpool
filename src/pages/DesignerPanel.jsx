  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        // super_admin, admin & designer: auto-approve
        approved:    ["super_admin","admin","designer"].includes(profile.role?.toLowerCase?.()),
        approved_by: ["super_admin","admin","designer"].includes(profile.role?.toLowerCase?.()) ? profile.id : null,
      ...
      if (editItem) {
        ...
        if (error) throw error;
        notify(
          ["super_admin","admin","designer"].includes(profile.role?.toLowerCase?.())
            ? "✅ Item créé et publié !"
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                ...
                >
                  {uploading ? "⏫ UPLOAD EN COURS..." : saving ? "💾 SAUVEGARDE..." :
                   ...
                   ["super_admin","admin","designer"].includes(profile?.role?.toLowerCase?.()) ? "🚀 CRÉER ET PUBLIER" : "📤 SOUMETTRE POUR APPROBATION"}
                </button>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
                            ...
                            {!item.approved && ["admin","super_admin","designer"].includes(profile?.role?.toLowerCase?.()) && (
                              <button
                                onClick={async () => {
                                  await supabase.from("store_items")
