...
export default function AdminStorePanel() {
  ...
  const fetchUsers = async () => {
    const { data } = await supabase
      .from("profiles")
      .select("id, full_name, email, role, avatar_url, level, equipped_badge, equipped_name_color")
      .order("full_name");
    setUsers(data || []);
  };
  ...
  const handleGrant = async () => {
    ...
    notify(`✅ ${selectedItem.name} accordé à ${selectedUser.full_name}`);
    addLog("GRANT ITEM", selectedUser.full_name, `${selectedItem.name} (${selectedItem.rarity})`);
  ...
  const handleRemove = async (inv) => {
    if (!confirm(`Retirer "${inv.item?.name}" à ${selectedUser.full_name} ?`)) return;
    setLoading(true);
    ...
    notify(`🗑️ ${inv.item?.name} retiré de ${selectedUser.full_name}`);
    addLog("REMOVE ITEM", selectedUser.full_name, inv.item?.name);
    fetchUserInventory(selectedUser.id);
  };
  ...
  const handleCoins = async () => {
    ...
    notify(`💰 ${sign}${coinAmount} coins → ${selectedUser.full_name} (nouveau solde: ${data.new_balance})`);
    addLog(coinAmount > 0 ? "ADD COINS" : "REMOVE COINS", selectedUser.full_name, `${sign}${coinAmount} — ${coinReason}`);
    setCoinAmount(0);
    setCoinReason("");
              ...
              >
                ...
                {filteredUsers.map(u => (
                  <option key={u.id} value={u.id}>
                    {u.full_name} ({u.role}) {u.email ? `— ${u.email}` : ""}
                  </option>
                ))}
              </select>
            </div>
                ...
                {selectedUser && userInventory.length > 0 && (
                  <div style={{ marginTop: 16, padding: "16px", borderRadius: 14, background: "#08031a", border: "1px solid rgba(255,255,255,0.07)" }}>
                    <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1.5, color: "rgba(255,255,255,0.4)", marginBottom: 12 }}>
                      INVENTAIRE DE {selectedUser.full_name?.toUpperCase()} ({userInventory.length})
                    </div>
