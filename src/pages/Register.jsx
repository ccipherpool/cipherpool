...
import {
  ...
  Instagram, Youtube, MessageCircle, Music2, ChevronRight,
  ChevronLeft, Check, Zap, AlertCircle,
} from "lucide-react";
...
const STEP_LABELS = ["Account", "Player Info", "Socials"];
...
export default function Register() {
  ...
  const validateStep2 = () => {
    ...
    else if (!/^\d+$/.test(uid.trim())) e.uid = "UID must be numeric";
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: 24, position: "relative", overflow: "hidden", background: "#060810" }}>
      ...
      <motion.div
        ...
        style={{ position: "relative", zIndex: 20, width: "100%", maxWidth: 420 }}
      ...
      >
        <div style={{
          ...
          <div style={{ padding: "28px 28px 20px", textAlign: "center" }}>
            <div style={{ width: 44, height: 44, borderRadius: 14, background: "linear-gradient(135deg, #6366f1, #06b6d4)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 14px", boxShadow: "0 8px 24px rgba(99,102,241,0.4)" }}>
              <Zap size={20} style={{ color: "#fff" }} />
            </div>
            <h1 style={{ fontSize: 22, fontWeight: 800, color: "#f1f5f9", margin: "0 0 4px", fontFamily: "'Space Grotesk', sans-serif", letterSpacing: -0.3 }}>
              Create Your Profile
            </h1>
            <p style={{ fontSize: 13, color: "rgba(255,255,255,0.35)", margin: 0 }}>
              Join the CipherPool arena
            </p>
          </div>
            ...
            <AnimatePresence mode="wait">
              ...
              {step === 1 && (
                <motion.div key="s1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.22 }}>
                  <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                    <Input
                      icon={User} placeholder="Username (min 3 chars)"
              ...
              {step === 2 && (
                <motion.div key="s2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.22 }}>
                  <div style={{ padding: "10px 12px", borderRadius: 10, background: "rgba(99,102,241,0.07)", border: "1px solid rgba(99,102,241,0.15)", marginBottom: 14 }}>
                    <p style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", margin: 0, lineHeight: 1.5 }}>
                      <span style={{ color: "#a5b4fc", fontWeight: 700 }}>Free Fire UID</span> and{" "}
                      <span style={{ color: "#a5b4fc", fontWeight: 700 }}>Age</span> are required to participate in tournaments.
                    </p>
                  </div>

                  <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                    <Input
                      icon={Hash} placeholder="Free Fire UID (numbers only)"
                      value={uid} onChange={e => setUid(e.target.value.replace(/\D/, ""))} required
                      error={fieldErrors.uid}
                      iconColor="#f59e0b"
                    />
                    <Input
                      icon={MapPin} placeholder="Country (e.g. MA, DZ, TN)"
              ...
              {step === 3 && (
                <motion.div key="s3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.22 }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                    <Input icon={Instagram}     placeholder="Instagram URL (optional)" value={instagram} onChange={e => setInstagram(e.target.value)} iconColor="#e1306c" />
                    ...
                    <Input icon={Youtube}       placeholder="YouTube URL (optional)"   value={youtube}   onChange={e => setYoutube(e.target.value)}   iconColor="#ff0000" />
                  </div>
