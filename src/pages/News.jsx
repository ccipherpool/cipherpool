...
const CYAN="#00d4ff",INDIGO="#818cf8",VIOLET="#a78bfa",GREEN="#10b981",BG="#020817",CARD="#0a1628";
const cx = a => `rgba(0,212,255,${a})`;

const CAT = {
  general:    { label:"GÉNÉRAL",     color:INDIGO, bg:"rgba(129,140,248,.1)" },
  tournament: { label:"TOURNOIS",    color:"#fbbf24", bg:"rgba(251,191,36,.1)" },
...
const DUMMY = [
  { id:"1", title:"Saison 3 : Nouvelles récompenses et tournois inédits",      excerpt:"La saison 3 de CipherPool arrive avec des lots exceptionnels et de nouveaux formats.",      category:"tournament", featured:true,  views:1240, published_at:new Date(Date.now()-3600000*2).toISOString(),  cover_url:null, tags:["saison3","tournois"] },
  { id:"2", title:"Mise à jour v2.4 : Système de teams amélioré",              excerpt:"Le système d'équipes reçoit une refonte complète avec invitations et statistiques.",          category:"update",     featured:false, views:876,  published_at:new Date(Date.now()-3600000*18).toISOString(), cover_url:null, tags:["teams","update"] },
  { id:"3", title:"Spotlight joueur : XxShadowKillxX domine le classement",    excerpt:"Découvrez l'histoire du meilleur joueur de la saison avec une interview exclusive.",        category:"player",     featured:false, views:654,  published_at:new Date(Date.now()-3600000*36).toISOString(), cover_url:null, tags:["player","interview"] },
  { id:"4", title:"Les Achievements arrivent sur CipherPool",                  excerpt:"Gagnez des badges, débloquez des succès rares et réclamez des récompenses.",               category:"update",     featured:false, views:432,  published_at:new Date(Date.now()-86400000*2).toISOString(), cover_url:null, tags:["achievements"] },
  { id:"5", title:"Résultats Tournoi Septembre : Palmarès complet",             excerpt:"Retrouvez tous les résultats, kills et classements du grand tournoi mensuel.",             category:"tournament", featured:false, views:1089, published_at:new Date(Date.now()-86400000*3).toISOString(), cover_url:null, tags:["results"] },
  { id:"6", title:"CipherPool s'associe avec de nouveaux partenaires",         excerpt:"De nouvelles collaborations pour enrichir l'expérience et multiplier les lots.",           category:"general",    featured:false, views:321,  published_at:new Date(Date.now()-86400000*5).toISOString(), cover_url:null, tags:["partner"] },
];
...
export default function News() {
  ...
  const fetchNews = async () => {
    setLoading(true);
    try {
      // ✅ FIX: utilise published_at (pas "published" qui n'existe pas)
      const { data } = await supabase
        .from("news")
        .select("*")
        .not("published_at", "is", null)   // ← corrigé
  ...
  const openArticle = (a) => {
    setSelected(a);
    // Incrémenter vues sans bloquer
    supabase.from("news").update({ views: (a.views || 0) + 1 }).eq("id", a.id).catch(() => {});
  };
      ...
      {featured && (
        <motion.div initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }} whileHover={{ y:-3 }}
          ...
          <div style={{ maxWidth:680 }}>
            ...
            <h2 style={{ fontFamily:"Bebas Neue,cursive", fontSize:34, letterSpacing:2, color:"#fff", lineHeight:1.15, margin:"10px 0 12px" }} className="news-title">
              {featured.title}
            </h2>
            ...
            <div style={{ display:"flex", gap:16, alignItems:"center" }}>
              ...
              <span style={{ fontFamily:"JetBrains Mono,monospace", fontSize:9, color:"rgba(255,255,255,.25)" }}>👁 {featured.views || 0}</span>
        ...
        <div style={{ display:"flex", gap:4, background:cx(.04), borderRadius:10, padding:4 }}>
          {[["all","TOUS"],["tournament","TOURNOIS"],["update","UPDATES"],["player","JOUEURS"],["team","ÉQUIPES"],["general","GÉNÉRAL"]].map(([k,l]) => (
            <button key={k} onClick={() => setFilter(k)}
              style={{ padding:"7px 14px", borderRadius:7, border:"none", cursor:"pointer", fontFamily:"JetBrains Mono,monospace", fontSize:8, letterSpacing:1.5, background:filter===k?`linear-gradient(135deg,${CYAN},${INDIGO})`:"transparent", color:filter===k?"#000":"rgba(255,255,255,.4)", fontWeight:filter===k?700:400, transition:"all .2s" }}>
              {l}
            </button>
          ))}
        </div>
        ...
        {filtered.map((a, i) => {
          ...
          return (
            <motion.div key={a.id} initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} transition={{ delay:i*.05 }}
              ...
              className="news-card"
              style={{ cursor:"pointer", position:"relative", overflow:"hidden", background:CARD, border:`1px solid ${cx(.09)}`, borderRadius:14, padding:"22px 22px", boxShadow:"0 4px 20px rgba(0,0,0,.4)", transition:"border-color .22s" }}>
              ...
              <div style={{ display:"flex", gap:12, alignItems:"center" }}>
                ...
                <span style={{ fontFamily:"JetBrains Mono,monospace", fontSize:8, color:"rgba(255,255,255,.2)" }}>👁 {a.views || 0}</span>
          ...
          <motion.div initial={{ y:40, opacity:0 }} animate={{ y:0, opacity:1 }} exit={{ y:40, opacity:0 }}
            ...
            <div style={{ padding:"clamp(14px,4vw,28px) clamp(14px,4vw,32px)" }}>
              ...
              <div style={{ display:"flex", gap:14, marginBottom:24, flexWrap:"wrap" }}>
                ...
                <span style={{ fontFamily:"JetBrains Mono,monospace", fontSize:9, color:"rgba(255,255,255,.25)" }}>👁 {selected.views || 0} vues</span>
                {selected.tags?.map(t => (
                  <span key={t} style={{ fontFamily:"JetBrains Mono,monospace", fontSize:8, color:cx(.5), background:cx(.06), border:`1px solid ${cx(.1)}`, padding:"2px 8px", borderRadius:5 }}>#{t}</span>
                ))}
              </div>
