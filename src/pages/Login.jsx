    ...
    <div style={{ marginBottom: 18 }}>
      ...
      <div style={{ position:"relative" }}>
        <input
          ...
          style={{ width:"100%", boxSizing:"border-box", padding: isPwd ? "12px 44px 12px 16px" : "12px 16px", borderRadius:12, border:`1px solid ${error ? C.danger+"66" : C.border}`, background:"rgba(255,255,255,0.04)", color:C.text, fontFamily:"'Space Grotesk',sans-serif", fontSize:14, outline:"none", transition:"all .22s" }}
        ...
        />
        {isPwd && (
          <button type="button" onClick={() => setShow(s => !s)} style={{ position:"absolute", right:12, top:"50%", transform:"translateY(-50%)", background:"none", border:"none", cursor:"pointer", fontSize:16, opacity:.45, color:"#fff" }}>
            {show ? "🙈" : "👁"}
          </button>
        )}
      </div>
...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (Date.now() < lockedUntil.current) {
      setGlobal(`Trop de tentatives. Réessaie dans ${Math.ceil((lockedUntil.current - Date.now()) / 1000)}s.`);
      return;
    }
        ...
        if (attempts.current >= MAX_ATTEMPTS) {
          ...
          const msg =
            error.message.includes("Invalid login")
              ? `Email ou mot de passe incorrect. ${remaining} essai${remaining > 1 ? "s" : ""} restant${remaining > 1 ? "s" : ""}.`
        ...
        <div style={{ background:"rgba(15,15,23,0.92)", backdropFilter:"blur(24px)", border:`1px solid ${C.border}`, borderRadius:22, padding:"clamp(20px,5vw,32px) clamp(16px,4vw,30px)", boxShadow:"0 24px 80px rgba(0,0,0,0.6), 0 0 0 1px rgba(255,255,255,0.04)" }}>
          ...
          {isLocked && (
            <motion.div initial={{ opacity:0, y:-8 }} animate={{ opacity:1, y:0 }}
              ...
              <div>
                ...
                <p style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:10, color:"rgba(255,71,87,0.7)", margin:"3px 0 0" }}>Réessaie dans <strong>{lockSecs}s</strong></p>
              </div>
          ...
          <form onSubmit={handleSubmit} noValidate>
            ...
            <motion.button
              ...
              style={{ width:"100%", padding:"14px", borderRadius:13, background: isLocked ? "rgba(255,255,255,0.06)" : `linear-gradient(135deg,${C.primary},#4f46e5)`, border:"none", color: isLocked ? "rgba(255,255,255,0.25)" : "#fff", fontFamily:"'JetBrains Mono',monospace", fontSize:12, letterSpacing:2, fontWeight:700, cursor: loading || isLocked ? "not-allowed" : "pointer", boxShadow: isLocked ? "none" : `0 8px 28px ${C.primaryGlow}`, display:"flex", alignItems:"center", justifyContent:"center", gap:10, transition:"all .25s" }}
            >
              {loading ? (
                <>
              ...
              ) : isLocked ? `🔒 BLOQUÉ (${lockSecs}s)` : "⚡ SE CONNECTER"}
            </motion.button>
          </form>
