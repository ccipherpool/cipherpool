-- ══════════════════════════════════════════════════════════════════════
-- CipherPool — sql/50 Advanced Notification System v2
-- Tables: notification_broadcasts, notification_preferences
-- Alters: notifications (category, priority, image_url, created_by)
-- RPCs: send_global_notification, send_user_notification,
--       mark_notification_read, mark_all_notifications_read,
--       get_unread_notification_count
-- Realtime: notification_broadcasts
-- Safe to run multiple times (fully idempotent).
-- Depends on: sql/39_realtime_notifications.sql
-- ══════════════════════════════════════════════════════════════════════

-- ─────────────────────────────────────────────────────────────────────
-- 1. ALTER notifications — add missing columns
-- ─────────────────────────────────────────────────────────────────────
DO $$
BEGIN
  -- category: tournament | social | admin | system | marketing | announcement
  IF NOT EXISTS (
  ...
  -- image_url: optional banner/avatar image
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
  ...
  -- created_by: which admin sent this (null = system)
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
  ...
  -- metadata: extra structured data (supercedes old data column usage)
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
...
-- Relax the type CHECK constraint to allow all broadcast types
-- (Postgres auto-names it notifications_type_check)
DO $$
BEGIN
  BEGIN
...
-- ─────────────────────────────────────────────────────────────────────
-- 2. notification_broadcasts — record of every admin broadcast
-- ─────────────────────────────────────────────────────────────────────
...
CREATE INDEX IF NOT EXISTS nb_created_at_idx ON public.notification_broadcasts(created_at DESC);
CREATE INDEX IF NOT EXISTS nb_created_by_idx ON public.notification_broadcasts(created_by);
...
CREATE INDEX IF NOT EXISTS np_user_idx ON public.notification_preferences(user_id);
...
-- ─────────────────────────────────────────────────────────────────────
-- 6. RPC: mark_notification_read — mark a single notification read
-- ─────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────
-- 7. RPC: mark_all_notifications_read — mark all read for current user
-- ─────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────
-- 8. RPC: send_user_notification — send to a single user
-- ─────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  INSERT INTO public.notifications (
    ...
    p_icon, p_action_url, p_image_url, false, v_caller_id,
...
-- ─────────────────────────────────────────────────────────────────────
-- 9. RPC: send_global_notification — fan-out broadcast to target audience
-- ─────────────────────────────────────────────────────────────────────
DROP FUNCTION IF EXISTS public.send_global_notification(text,text,text,text,text,text,text,text,text,jsonb);
...
BEGIN
  ...
  -- Fan-out: one notification per target user
  IF v_user_ids IS NOT NULL AND array_length(v_user_ids, 1) > 0 THEN
    ...
    SELECT
      uid, p_title, p_message, p_type, p_category, p_priority,
      p_icon, p_action_url, p_image_url, false, v_caller_id,
...
-- ─────────────────────────────────────────────────────────────────────
-- 10. RPC: upsert_notification_preferences
-- ─────────────────────────────────────────────────────────────────────
...
CREATE INDEX IF NOT EXISTS notif_expires_idx    ON public.notifications(expires_at) WHERE expires_at IS NOT NULL;

-- ─────────────────────────────────────────────────────────────────────
-- 12. Register migration
-- ─────────────────────────────────────────────────────────────────────
INSERT INTO public.schema_migrations (filename)
VALUES ('50_notification_system_v2.sql')
