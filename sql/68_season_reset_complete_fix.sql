...
--
-- BUGS FIXED IN start_new_season_v2:
--   1. notifications INSERT used "body" — correct column is "message"
--   2. wallet_transactions used "description" + invalid type "admin_adjustment"
--   3. clear_clan_chats / clear_team_chats did nothing (tables never deleted)
--   4. All UPDATE/DELETE now have WHERE clause (Supabase safety requirement)
--   5. is_admin() / is_team_member() RLS functions re-ensured
...
-- ─────────────────────────────────────────────────────────────────────
-- 2. teams table — open SELECT policy for authenticated users
...
-- ─────────────────────────────────────────────────────────────────────
-- 3. Drop ALL overloads of start_new_season / start_new_season_v2
-- ─────────────────────────────────────────────────────────────────────
DO $$
DECLARE r record;
BEGIN
...
-- ─────────────────────────────────────────────────────────────────────
-- 4. start_new_season_v2 — fully fixed version
-- ─────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  IF (v_cfg->>'wipe_everything')::boolean IS TRUE THEN
    v_cfg := v_cfg || '{
      ...
      "unequip_avatars":true,"reset_ready_status":true
    }'::jsonb;
  END IF;
  ...
  IF v_old_season_id IS NOT NULL THEN
    BEGIN
      ...
      SELECT
        v_old_season_id, p.id,
        ROW_NUMBER() OVER (ORDER BY COALESCE(ps.total_points,0) DESC, p.level DESC, p.xp DESC)::int,
        COALESCE(ps.total_points,0), COALESCE(p.level,1), COALESCE(w.balance,0),
  ...
  IF (v_cfg->>'reset_coins')::boolean IS TRUE OR (v_cfg->>'reset_wallet_balances')::boolean IS TRUE THEN
    BEGIN
      -- FIX: correct column "reason" (was "description") + valid type "debit" (was "admin_adjustment")
      INSERT INTO public.wallet_transactions (user_id, amount, type, reason)
      SELECT user_id, -balance, 'debit', 'Season ' || v_season_number || ' reset'
        FROM public.wallets WHERE balance > 0;
    EXCEPTION WHEN OTHERS THEN
      v_log := array_append(v_log, '[SEASON RESET] wallet_tx skipped: ' || SQLERRM);
    END;
    -- WHERE true required by Supabase safety check
  ...
  THEN
    UPDATE public.player_stats SET
      kills              = CASE WHEN (v_cfg->>'reset_kd')::boolean             IS TRUE THEN 0 ELSE kills END,
  ...
  THEN
    UPDATE public.clans SET
      points     = CASE WHEN (v_cfg->>'reset_clan_points')::boolean IS TRUE
  ...
  -- Tournament chats (room_messages)
  ...
  -- FIX: clan_messages now actually deleted
  IF (v_cfg->>'clear_clan_chats')::boolean IS TRUE THEN
    ...
    EXCEPTION WHEN OTHERS THEN
      v_log := array_append(v_log, '[SEASON RESET] Clan chat skipped: ' || SQLERRM);
    END;
  END IF;

  -- FIX: team_messages now actually deleted
  IF (v_cfg->>'clear_team_chats')::boolean IS TRUE THEN
    BEGIN
      DELETE FROM public.team_messages WHERE true;
      v_resets_applied := v_resets_applied + 1;
      v_log := array_append(v_log, '[SEASON RESET] Team chats cleared ✓');
    EXCEPTION WHEN OTHERS THEN
      v_log := array_append(v_log, '[SEASON RESET] Team chat skipped: ' || SQLERRM);
    END;
  END IF;
  ...
  INSERT INTO public.seasons (
    number, name, description, status, start_date, updated_at, reset_config
  ) VALUES (
    v_season_number, trim(p_name), p_description, 'active', now(), now(), v_cfg
  )
  RETURNING id INTO v_new_season_id;

  v_log := array_append(v_log, format('[SEASON RESET] Season %s created ✓', v_season_number));

  -- Send notifications
  -- FIX: correct column "message" (was "body") + correct type "season_start"
  BEGIN
  ...
  BEGIN
    ...
    VALUES (v_new_season_id, 'season_started', auth.uid(),
      jsonb_build_object(
        'season_name', p_name, 'season_number', v_season_number,
        'closed_season', v_old_season_id, 'snapshots_taken', v_snapshot_ct,
        'resets_applied', v_resets_applied, 'config', v_cfg
      ));
  EXCEPTION WHEN OTHERS THEN
...
EXCEPTION WHEN OTHERS THEN
  RETURN jsonb_build_object(
    'success', false, 'error', SQLERRM, 'detail', SQLSTATE,
    'log', to_json(v_log)
  );
END;
$$;
...
-- ─────────────────────────────────────────────────────────────────────
-- 5. start_new_season wrapper (backwards compat for SeasonsTab)
-- ─────────────────────────────────────────────────────────────────────
...
-- 6. Grants
-- ─────────────────────────────────────────────────────────────────────
GRANT EXECUTE ON FUNCTION public.start_new_season_v2(text, text, jsonb)   TO authenticated;
GRANT EXECUTE ON FUNCTION public.start_new_season(
  text, integer, text, boolean, boolean, boolean, boolean, boolean, boolean, boolean, boolean
) TO authenticated;
...
BEGIN
  SELECT COUNT(*) INTO v1 FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
  WHERE n.nspname = 'public' AND p.proname = 'start_new_season';
  SELECT COUNT(*) INTO v2 FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
